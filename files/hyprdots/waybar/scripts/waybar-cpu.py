...
for i, usage in enumerate(per_core):
    if i not in per_core_history:
        ...
        per_core_history[i] = (per_core_history[i] * decay_factor) + (
            usage * (1 - decay_factor)
        )
...
max_line_len = (
    max(len(re.sub(r"<.*?>", "", line_text)) for _, line_text in cpu_rows) + 5
)
...
tooltip_lines.append("")
tooltip_lines.append(
    f"{center_padding}  <span foreground='{border_color}'>╭──┘└────┘⠿└─────┘└─╮</span>"
)
tooltip_lines.append(
    f"{center_padding}  <span foreground='{border_color}'>┘</span><span foreground='{substrate_color}'>░░░░░░░░░░░░░░░░░░░</span><span foreground='{border_color}'>└</span>"
)
...
for row in range(6):
    ...
    line_parts = [
        f"{center_padding}  <span foreground='{border_color}'>{start_char}</span><span foreground='{substrate_color}'>░░</span>"
    ]
    for col in range(4):
        core_idx = row * 4 + col
        if core_idx < len(per_core):
            ...
            line_parts.append(
                f"<span foreground='{border_color}'>[</span><span foreground='{color}'>{circle}</span><span foreground='{border_color}'>]</span>"
            )
        else:
        ...
        if col < 3:
            line_parts.append(f"<span foreground='{substrate_color}'>░</span>")
    line_parts.append(
        f"<span foreground='{substrate_color}'>░░</span><span foreground='{border_color}'>{end_char}</span>"
    )
    ...
    for line in ps_output.split("\n"):
        ...
        if len(parts) >= 2:
            try:
                ...
                color = get_core_color(usage)
                tooltip_lines.append(
                    f" • {name:<15} <span foreground='{color}'> {usage:>5.1f}%</span>"
                )
                count += 1
            except:
                continue
...
tooltip_lines.append("")
tooltip_lines.append(
    f"<span foreground='{COLORS['white']}'>{'┈' * max_line_len}</span>"
)
